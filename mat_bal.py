...
for j in range(1,ny+1):
    for i in range(1,nx+1):
        ...
        bo= bob*(1-co*(pnow-pb))
        block_np=((block_stoiip*boi*ce)*(pi-pnow))/bo
        total_np = total_np + block_np
        print('The amount of oil in Block {0:.0f} is {1:.2f} STB'.format(block_position, block_stoiip))
        print('The amount of cummulative oil produced in Block {0:.0f} is {1:.2f} STB'.format(block_position, block_np))
...
print('The total cummulative oil produced from the reservoir is {0:.2f} STB'.format(total_np))                       
